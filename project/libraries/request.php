<?php
    ...
    if( array_key_exists('widgets',$_REQUEST) || ( array_key_exists('widget',$_REQUEST) && empty($_REQUEST['widget']) ) ) {
        $list = array( 'fields' => array( 'widget' ), 'rows' => array() );
        chdir($widgetDefinitionsDir);
        $widgetDefinitions = glob("*.ddf", GLOB_BRACE);
        if( $debug ) {
            $list['debug'] = array( 'cwd' => getcwd(), 'defs' => print_r($widgetDefinitions,true) );
        }
        ...
        if( $debug ) {
            $list['debug'] = array( 'cwd' => getcwd(), 'widgets' => print_r($widgetFilePaths,true) ); 
        }
        ...
        // Here's some replicated code that could be turned into a function
        $list = array( 'fields' => array( 'template' ), 'rows' => array(), 'errors' => array() );
        $templates = explode ( ',', @$_REQUEST['template'] );
        if( $debug ) {
            $list['debug'] = array( 'cwd' => getcwd(), 'templates' => print_r($templates,true) );
        }
        foreach( $templates as $name ) {
            $template = @file_get_contents( $templatesDir . "/{$name}.tpl" );
            if( ! empty($template) ) {
                array_push($list['rows'], array( 'template' => $template ) );
            } else {
                array_push($list['errors'], array( 'template' => $name, 'message' => "File was empty" ));
            }
        }
        // Here's some replicated code that could be turned into a function
        $list['count'] = count($list['rows']);
